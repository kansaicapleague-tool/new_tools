...
let imagePath = "kantoCStandingsBackground.jpg"; // 背景画像の取得
...
function changeClick() {
    background.addEventListener("load", function (){
        ...
        for (var i = 0; i < teamNum; i++) {
            ...
            ctx.textAlign = "left";
            ctx.fillText(teamName[i], 260, 368 + (159 * i));
            ctx.fillStyle = "#FFFFFF";
            ctx.font = "100px Zen Kaku Gothic New";
            ctx.textAlign = "center";
            ctx.fillText(teamGame[i], 1137, 370 + (159 * i));
            ctx.fillText(teamWin[i], 1302, 370 + (159 * i));
            ctx.fillText(teamLose[i], 1470, 370 + (159 * i));
            ctx.fillText(teamDraw[i], 1638, 370 + (159 * i));
            ctx.fillText(teamPoint[i], 1806, 370 + (159 * i));
        }
        display(teamLogo);
    });
